...
new_Alice <- read.csv("SOTA_Data_Product.csv")
new_Alice <- new_Alice %>% filter(HI >= 0)


Crypto_pull <- new_Alice %>% count(Latitude, Longitude) %>% mutate(mus_caught = n) %>% select(Latitude, Longitude, mus_caught) %>% arrange(mus_caught)
Crypto_pull_pos <- new_Alice %>% filter(Ct_mean > 0) %>% count(Latitude, Longitude) %>% mutate(Crypto_mus_caught = n) %>% select(Latitude, Longitude, Crypto_mus_caught) %>% arrange(Crypto_mus_caught)
new_Alice_1 <- left_join(new_Alice, Crypto_pull)
new_Alice_2 <- left_join(new_Alice_1, Crypto_pull_pos)
new_Alice <- new_Alice_2
...
new_Alice <- new_Alice %>% filter(HI  != "NA", Longitude != "NA", Latitude  != "NA")
new_Alice <- new_Alice %>% replace_na(list(Crypto_mus_caught = 0))
new_Alice[,'HI']=format(round(new_Alice[,'HI'],2),nsmall=2)

new_Alice$mus_Level <-  cut(new_Alice$mus_caught, c(0, 1, 5, 10, 15, 20, 30), include.lowest = T , labels = c('1 mouse', 'up to 5 mice', 'up to 10 mice', 'up to 15 mice', 'up to 20 mice', ' up to 30 mice'))

new_Alice$HI <- as.numeric(new_Alice$HI)
data_col_HI       = colorFactor(beach(6), new_Alice$HI)
data_col_new_Alice    = colorFactor(beach(6), new_Alice$HI)

data_col_HI_Level = colorFactor(beach(6), new_Alice$HI_Level)


new_Alice$HI_Level <-  cut(new_Alice$HI, c(0, 0.001, 0.250, 0.500, 0.750, 0.999, 1), include.lowest = T , labels = c('HI = 0', 'HI < 0.25', 'HI < 0.5', 'HI < 0.75', 'HI < 1', 'HI = 1'))

HI_0 <- new_Alice %>% filter(HI < 0.01)
HI_below_0.25 <- new_Alice %>% filter(HI > 0.01, HI <= 0.25)
HI_below_0.5 <- new_Alice %>% filter(HI > 0.25, HI <= 0.5)
HI_below_0.75 <- new_Alice %>% filter(HI > 0.5, HI <= 0.75)
HI_below_1 <- new_Alice %>% filter(HI > 0.75, HI < 1)
HI_equal_1 <- new_Alice %>% filter(HI > 0.99)
...
mus_30 <- Crypto_Detection %>% filter(mus_caught <= 30)
