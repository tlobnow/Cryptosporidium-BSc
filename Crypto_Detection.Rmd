...
map %>%
  ...
  addLegend("bottomleft", 
            ...
            opacity = 1) %>%
  addLayersControl(overlayGroups = c('all Samples', 'qPCR Samples'),
...
The Standard Curve would then be used in a linear model to derive Oocyst amounts from the Ct values observed during qPCR. Lower Ct values suggest higher initial Oocyst Amounts. Blue area in the plot depicts higher density areas, so most wild samples retrieve relatively low parasite burden, few have much higher burden.
...
Crypto_Detection_1 <- left_join(Crypto_Detection, df)

df <- df %>% mutate(Top_Location = Crypto_per_Location >= 3)

Crypto_Top <- df %>% filter(Top_Location == T)
Crypto_Flop <- df %>% filter(Top_Location == F)

data_col_Top   = colorFactor(beach(2), df$Top_Location)
...
map %>%
  addCircleMarkers(data = Crypto_Top, 
                   col = ~data_col_Top(Top_Location),
                   label = ~htmlEscape(Crypto_per_Location),
                   popup = ~paste("<b>Location:<b>", as.character(Latitude), "<b>,<b>", as.character(Longitude), "<br>",
                                  "<b>Crypto per Location:<b>", as.character(Crypto_per_Location), "<br>",
                                  sep=" "),
                   radius = 3, 
                   opacity = 1,
                   group = "Crypto_Top") %>%
  addCircleMarkers(data = Crypto_Flop, 
                   col = ~data_col_Top(Top_Location),
                   label = ~htmlEscape(Crypto_per_Location),
                   popup = ~paste("<b>Location:<b>", as.character(Latitude), "<b>,<b>", as.character(Longitude), "<br>",
                                  "<b>Crypto per Location:<b>", as.character(Crypto_per_Location), "<br>",
                   ...
                   opacity = 0.2,
                   group = "Crypto_Flop") %>%
  addLayersControl(overlayGroups = c("Crypto_Top", 
                                  "Crypto_Flop"),
                   options = layersControlOptions(collapsed = F))
